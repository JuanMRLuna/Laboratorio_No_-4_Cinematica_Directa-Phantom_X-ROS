...
class DynamixelGUI:
    def __init__(self, root):
        ...
        self.root.geometry("1200x900")  # Aumenté la altura de la ventana
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(
            self.root,
            text="Control de Brazo Robótico Dynamixel",
            font=("Arial", 16, "bold"),
            bg='#f0f0f0',
            fg='#333333'
        )
        ...
        tk.Label(connection_frame, text="Puerto:", bg='#f0f0f0').grid(row=0, column=0, padx=5)
        self.port_entry = tk.Entry(connection_frame, width=15)
        self.port_entry.insert(0, self.port_name)
        self.port_entry.grid(row=0, column=1, padx=5)
       
        tk.Label(connection_frame, text="Baudrate:", bg='#f0f0f0').grid(row=0, column=2, padx=5)
        ...
        # Frame para posiciones actuales - MEJORADO
    ...
    def connect_to_dynamixel(self):
        try:
            self.port_name = self.port_entry.get()
            self.baudrate = int(self.baudrate_entry.get())
    ...
    def move_servos(self, goal_positions, config_name):
        try:
            ...
            self.root.after(0, lambda: self.status_label.config(
                text=f"Estado: Ejecutando {config_name}...", fg='orange'
            ))
            ...
            for i, (dxl_id, goal) in enumerate(zip(self.dxl_ids, goal_positions)):
                ...
                self.root.after(0, lambda motor=i+1: self.status_label.config(
                    text=f"Estado: Moviendo Motor {motor} - {config_name}", fg='orange'
                ))
            ...
            self.root.after(0, lambda: self.status_label.config(
                text="Estado: Conectado", fg='green'
            ))
